...
if (isset($_GET['machine_id'])) {
  $id = (int)$_GET['machine_id'];
  $machine = get_machine_by_id($con, $id);
  $title = $machine['title'];
  $description = $machine['description'];
  $img = $machine['image_path_1'];
  ...
  $page_content = include_template('machine_id.php', [
    'title' => $title,
    'machine' => $machine,
    'action' => '#contact',
  ...
  $title = 'Станки для шлифовки и полировки оптических деталей';
  ...
  $res = mysqli_query($con, "SELECT COUNT(*) as cnt FROM machines");
  ...
  $sql = "SELECT * FROM machines ORDER BY title LIMIT " . $page_items . " OFFSET " . $offset;

  $machines = mysqli_query($con, $sql);

  $page_content = include_template('machines.php', [
    'title' => $title,
    'machines' => $machines,
    'pages' => $pages,
